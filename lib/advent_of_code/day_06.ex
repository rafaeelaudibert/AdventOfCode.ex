...
defmodule AdventOfCode.Day06 do
  ...
  def part2(test_filename) do
    read_chunked_lines(test_filename)
      |> Enum.map(fn lines -> lines |> Enum.map(& &1 |> String.split("", trim: true) |> MapSet.new()) end)
      |> Enum.map(fn lines -> lines |> Enum.reduce(fn answers, acc -> MapSet.intersection(acc, answers) end)end)
